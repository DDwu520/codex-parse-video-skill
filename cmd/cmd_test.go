...
func TestFormatTextBatchShowsFailureWhenMetadataExists(t *testing.T) {
	info := &parser.VideoParseInfo{Title: "只有元数据"}
	items := []batchResult{
		{
			Input:  "https://weibo.com/123/example",
			Failed: true,
			Data:   info,
			ErrMsg: "无可下载的媒体文件",
		},
	...
	out := &bytes.Buffer{}
	if err := outputBatch(out, FormatText, items); err != nil {
		t.Fatalf("输出批量结果失败: %v", err)
	}
	if !strings.Contains(out.String(), "[失败]") {
		t.Fatalf("下载失败项必须显示失败状态，实际: %s", out.String())
	}
}
